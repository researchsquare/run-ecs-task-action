...
async function runTask(ecs, clusterName, taskDefArn, taskCount, waitForTask, waitForMinutes, taskOverrides) {
  ...
  core.info(`Task started. Watch this task's progress in the Amazon ECS console: https://console.aws.amazon.com/ecs/home?region=${aws.config.region}#/clusters/${clusterName}/tasks`);
...
async function run() {
  try {
    ...
    try {
        registerResponse = await ecs.describeTaskDefinition(taskDefinitionArn).promise();
    } catch (error) {
    ...
    await runTask(ecs, clusterName, taskDefArn, taskCount, waitForTask, waitForMinutes, taskOverrides);
  }
  catch (error) {
